...
class DonaldrussellAllSpider(scrapy.spiders.CrawlSpider):
    ...
    affiliate_reference = 'donaldrussell'
    custom_settings = {
    ...
    def parse_item(self, response):
        ...
        try:
            item['description'] = response.xpath('.//div[@class="product-desc"]//div[@class="std"]/text()').extract()[0].strip()
        except:
...
class DonaldrussellSingleSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs): 
      super(DonaldrussellSingleSpider, self).__init__(*args, **kwargs) 
      logging.info("got to spider init")
      crawl_urls = json.loads(kwargs.get('start_urls'))
      logging.info(crawl_urls)
      self.start_urls = crawl_urls
